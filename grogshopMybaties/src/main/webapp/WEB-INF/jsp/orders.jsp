    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <form class="form-group input-group" action="searchOrder.action" id="search-from-list">
                            <input name="abc" type="text" class="form-control" value="" placeholder="搜索相关订单">
                            <span class="input-group-btn">
                            <button  class="btn btn-default" id="getOrder">
                            ...
                            <table class="table table-bordered table-striped" id="order_list_table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sName">酒店名称</th>
                                        <th class="sPhone">酒店地址</th>
                                        <th class="sPhone">酒店联系电话</th>
                                        <th class="uName">消费者姓名</th>
                                        <th class="uPhone">消费者电话</th>
                                        <th class="checkin">入住时间</th>
                                        <th class="checkout">退房时间</th>
                                        <th class="status">状态</th>
                                        <th class="action"></th>
                                    </tr>
                                </thead>
                                <tbody>
                                <c:forEach items="${orderAll }" var="orders">
                                	<tr>
	                                	<td>${orders.id }</td>
	                                	<td>${orders.shopName }</td>
	                                	<td>${orders.address }</td>
	                                	<td>${orders.shopPhone }</td>
	                                	<td>${orders.userName }</td>
	                                	<td>${orders.userPhone }</td>
	                                	<td><fmt:formatDate value="${orders.checkIn_time}" pattern="yyyy-MM-dd "/></td>
	                                	<td><fmt:formatDate value="${orders.checkOut_time}" pattern="yyyy-MM-dd "/></td>
	                                	<td>${orders.orderStatus }</td>
	                                	<td class="action">
	                                		<c:if test="${orders.orderStatus == '已完成' }">
	                                			<a href="deleteOrder.action?id=${orders.id }" class="btn btn-default warning">删除</a>
	                                		</c:if>
	                                		<c:if test="${orders.orderStatus == '未入住' }">
	                                			<a href="checkIn.action?id=${orders.id }" class="btn btn-default warning">办理入住</a>
	                                		</c:if>
	                                		<c:if test="${orders.orderStatus == '已入住' }">
	                                			<a href="checkOut.action?id=${orders.id }"  class="btn btn-default warning">办理退房</a>
	                                		</c:if>
	                                	</td>
                                	</tr>
                                </c:forEach>
