...
<body>
                ...
                <div class="row">
                    <div class="col-lg-10">
                        <div class="form-group input-group" id="search-from-list">
                            <input name="name" type="text" class="form-control" placeholder="搜索相关商家">
                            <span class="input-group-btn">
                            <a class="btn btn-default"  id="searchShop">
                    ...
                    <div class="col-lg-10">
                            ...
                            <table class="table table-bordered table-striped" id="shops-info-table">
                                ...
                                <tbody>
                                <c:forEach items="${grogshopAndStatus }" var="list">
                                <tr>
                                	<td>${list.name }</td>
                                	<td>${list.phone }</td>
                                	<td>${list.address }</td>
                                	<td><fmt:formatDate value="${list.apply_time}" pattern="yyyy-MM-dd "/></td>
                                	<td>${list.status }</td>
                                	<td class="action">
                                		<c:if test="${list.status =='已通过' }">
                                            <a href="statusBreak.action?id=${list.id }"  class="btn btn-default warning" >中止</a>
                                		</c:if>
                                		<c:if test="${list.status =='未通过' }">
                                            <a href="statusPass.action?id=${list.id }"  class="btn btn-default" >通过</a>
                                		</c:if>
                                    </td>
                                 </tr>
                                </c:forEach>
    ...
    <script>
      $('#searchShop').click(function(){
    	  $('#searchShop').attr('href','searchShops.action?name='+$("[name='name']").val());
      });
